  ...
  stages {
    stage("build") {
      steps {
        ...
        sh "tools/mkimage -C none -A arm -T script -d mahalia-bootloader.boot.cmd boot.scr"
        sh "tools/mkimage -C none -A arm -T script -d mahalia-bootloader.boot-emmc.cmd boot-emmc.scr"
        sh 'mhamakedeb mahalia-bootloader.csv $(cat mahalia-bootloader.version) armhf'
